...
class predictor():
    ...
    def __init__(self):
        ...
        #self.model_path = "E:\hussein_model"
...
##################################################### CHOICE BUTTON CLICKED TAB 2 ####################################
    ...
    def choiceButtonClicked2(self, button):
        global d
        d = []
        # Output button click to CSV
        btn_geo = button.geometry()
        btn_pos = btn_geo.topLeft()
        csr_pos = self.tab1.mapFromGlobal(QCursor.pos())
        print("%s click,%u,%u,%u,%u,%u,%u,%u"%(button.objectName(), time.time()-self.start_time, csr_pos.x(), csr_pos.y(), btn_pos.x(), btn_pos.y(), btn_geo.width(), btn_geo.height()))
        d.append({button.objectName()})
        start_button_clicked = time.time()
    ...
    def lineEditEdited(self, lineedit):
        if (len(lineedit.text()) == 1 and lineedit.text() !="Q"):
            if self.start_time == None:
                ...
                print("Start Time: %d\n"%(self.start_time));
                csr_pos = self.tab1.mapFromGlobal(QCursor.pos())
                print("event,time,cursor_x,cursor_y,target_topleft_x,target_topleft_y,target_width,target_height")
                print("start,%u,%u,%u,0,0,0,0"%(0, csr_pos.x(), csr_pos.y()))
                d.append({csr_pos.x(), csr_pos.y()})
    
    choiceButtonClicked2()
    df = pd.DataFrame(d)
    df.to_csv('FittsLaw.csv', index=False)
...
if __name__ == "__main__":
    ...
    ui.setupUi(Tab3)
    Tab3.show()
    QCursor.setPos(Tab3.mapToGlobal(QPoint(0,0)))
    print(QCursor.pos())                        # Screen space pos
    print(Tab3.mapFromGlobal(QCursor.pos()))    # Window space pos
    sys.exit(app.exec())